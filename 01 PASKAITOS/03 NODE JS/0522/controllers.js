...
export async function addPhone(req, res) {
    ...
    const { rows } = await db.query(`insert into phones (brand, price, year) values ('${brand}', ${price}, ${year}) returning *`)
...
export async function updatePhoneById(req, res) {
    ...
    const response = await db.query(`update phones set brand = '${brand}', price = ${price}, year = ${year} where id = ${id} returning *`)
